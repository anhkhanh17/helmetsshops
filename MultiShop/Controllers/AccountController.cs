            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    try
                    {
                        ...
                        db.Customers.Add(cs);
                        db.SaveChanges();
                        Success(string.Format("<b>{0}</b> Tạo tài khoản thành công ", model.Email), true);
                    }
                    catch (Exception e)
                    {
                        ModelState.AddModelError("", e.InnerException);
                        Danger(string.Format("<b>{0}</b> Tạo thất bại", e.Message), false);
                        return View();
                    }
                ...
                else
                {
                    ...
                    var list = db.Customers.Where(s => s.Id == model.Id);

                    if (model.Password.Length < 6)
                    {
                        ModelState.AddModelError("", "Mật khẩu phải 6 ký tự trở lên!");
                    }
